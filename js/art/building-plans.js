...
function legacyPlans(z, t, K) {
  const { walled, doorAt, BRICK, CONC_WALL, RUST, ROOF: roof, C_ROOF,
    TIMBER: wood, AWNING, HOOK, STEP, GRASS } = K;
  const material = z === 1 ? BRICK : z === 2 ? CONC_WALL : RUST;
  ...
  // Zone 4 (meat) is retired: a market is placed, not zoned (js/art/market.js).
  throw new Error(`legacyPlans: no plans for zone ${z} tier ${t}`);
}
...
export function extraPlans(z, t, K) {
  const { walled, doorAt, BRICK, CONC_WALL, RUST, ROOF, C_ROOF,
    TIMBER, AWNING, HOOK, STEP, GRASS } = K;
