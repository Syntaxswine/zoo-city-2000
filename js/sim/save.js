...
const TILE_ARRAYS = ["terrain", "road", "zone", "maxTier", "tier", "civic", "civicSize", "burning", "rubble", "variant", "flooded", "wall", "use", "rail", "meat", "big", "theme", "since", "cam", "mansion"];
...
export function toPlain(world) {
  ...
  for (const k of TILE_ARRAYS) if ((k !== "civicSize" && k !== "mansion") || world[k].some(Boolean)) o[k] = Array.from(world[k]); // an all-zero civicSize or mansion is left out: the save and the hash of a town without them are unchanged
  return o;
}
