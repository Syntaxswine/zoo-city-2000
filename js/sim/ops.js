...
export function costOf(world, op) {
  ...
  switch (op.kind) {
    ...
    case "rail": {
      ...
      const lay = new Set();
      for (const i of line) {
        if (world.cam[i] || world.rail[i] || world.civic[i] || world.zone[i] || isBuilt(world, i)) continue;
    ...
    case "wall": {
      ...
      for (const i of op.tiles || []) {
        ...
        if (world.cam[i] || world.wall[i] || world.rail[i] === 2 || world.terrain[i] === TERRAIN.WATER || world.civic[i] || world.zone[i] || isBuilt(world, i)) continue;
...
function snapshot(world, tiles) {
  return tiles.map(({ i }) => ({
    ...
    tier: world.tier[i], civic: world.civic[i], civicSize: world.civicSize[i], rubble: world.rubble[i], wall: world.wall[i], use: world.use[i], rail: world.rail[i], big: world.big[i], theme: world.theme[i], cam: world.cam[i], mansion: world.mansion[i],
  }));
}
...
export function undo(world) {
  ...
  for (const s of u.snap) {
    ...
    world.tier[s.i] = s.tier; world.civic[s.i] = s.civic; world.civicSize[s.i] = s.civicSize; world.rubble[s.i] = s.rubble; world.wall[s.i] = s.wall; world.use[s.i] = s.use; world.rail[s.i] = s.rail; world.big[s.i] = s.big; world.theme[s.i] = s.theme; world.cam[s.i] = s.cam; world.mansion[s.i] = s.mansion;
  }
