...
export function yearlyFigures(world) {
  ...
  let stations = 0;
  let cams = 0;
  let farms = 0, cemeteries = 0, sanitationWorks = 0, garbageDepots = 0;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (world.civic[i] === CIVIC.CEMETERY) cemeteries++;
    if (world.civic[i] === CIVIC.SANITATION) sanitationWorks++;
    if (world.civic[i] === CIVIC.GARBAGE) garbageDepots++;
  ...
  return { farms, cemeteries, sanitationWorks, garbageDepots, incomeYr: Math.round(incomeYr), upkeepYr: Math.round(upkeepYr), cutYr: Math.round(cutYr), taxByClass, fc, fi, fm, roads, bridges, parks, largeParks, zoos, fireStations, policeStations, centres, markets, walls, rails, railBridges, stations, cams, licence, libraries, universities, galleries, amphitheaters };
}
