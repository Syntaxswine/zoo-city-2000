...
// the rule reads (not the tile's own), and the card says it in words.
{
  ...
  // ---- ONE implementation ---------------------------------------------------
  {
    ...
    // sixth arrived with wealth and class (SPEC §9f): wealth.raiseMansion sets
    // tier 3 on the nine tiles of a mansion, and lots.lotsTick calls it from
    // its own loop exactly as it calls blocks.mergeLots - the lots' window,
    // settled by the same settleDoors. No fourth settle; the decision is recorded.
    const groundWriters = [];
    ...
    check("access: exactly six modules move the ground a forecourt stands on — lots and blocks inside lotsTick, wealth's raiseMansion inside lotsTick's own loop (lots.js calls it as it calls mergeLots), events inside eventsTick (it razes buildings AND it can open new water), ops at the op, and worldgen before any of it — and the tick settles the door graph after each of the three windows; a seventh writer means a fourth settle to decide on",
    ...
    check("access: the OLD predicate is gone — `hasAccess` is nowhere under js/, and seven sim modules import `served`: blocks, census, events, justice, lots, police-actions and progression; ops uses shared touchesRoad placement",
      !anyHasAccess && served5.join(" ") === "blocks.js census.js events.js justice.js lots.js police-actions.js progression.js" && /touchesRoad/.test(readFileSync(path.join(ROOT,"js/sim/ops.js"),"utf8")),
...
// ---- Part A': actionable needs and Inspect bubbles (SPEC §14b) ------------
{
  ...
  // EVERY click tool's ghost is its own footprint, from the one table ops.js builds by (CIVIC_SIDE), and its sprite
  // resolves. A list in input.js that stopped at the centre drew a 1×1 diamond under a 2×2 Library and a 3×3 University
  // (session 18 found it while a placed estate still existed; the estate went, the table stayed).
  {
    ...
    check("palette: every click tool's placement ghost is its footprint from CIVIC_SIDE — the Library 2×2, the University 3×3 — and every ghost sprite resolves", !ghosts.some((x) => x.startsWith("BAD")), ghosts.join(" "));
...
// ---- Part P: one tool registry, left palette and collision-free keys -------------------------
{
  ...
  check("palette: twenty-eight accessible emoji buttons; pointer, click, cost preview and active state stay synchronized",
...
{ const { checkMigration } = await import("./check-migration.mjs"); checkMigration(check); } // SPEC §7.2, §7.4 — homed, burnedAt, the tent before the road, the push
...
// ---- verdict ----------------------------------------------------------------------
{
  ...
  check("the mansion: both variants, lit and marked, and their hi-res twins occupy nine tiles (SPEC §9f)",
    [art.mansion(0), art.mansion(1), art.hires(art.mansion(0)), art.hires(art.mansion(1))].every((s) => s && s.footprint[0] === 3 && s.footprint[1] === 3)
