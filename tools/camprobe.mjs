...
function staff(w, open, n, kind = "police") {
  let placed = 0;
  for (let pass = 0; pass < 2 && placed < n; pass++) {
    for (const [x0, y0] of open) {
      if (placed >= n) break;
      const tx = pass ? x0 + BLOCK - 3 : x0 + 1;
      const ty = pass ? y0 + BLOCK - 3 : y0 + 1;
      if (!inBounds(w, tx+2, ty+2)) continue;
      let occupied=false;for(let dy=0;dy<3;dy++)for(let dx=0;dx<3;dx++)if(w.civic[(ty+dy)*w.w+tx+dx])occupied=true;
      if(occupied)continue;
      w.cash = 100000;
      apply(w, { kind: "bulldoze", x0: tx, y0: ty, x1: tx+2, y1: ty+2 });
      if (apply(w, { kind, tx, ty }).ok) placed++;
    }
  }
  w.cash = 100000;
  return placed;
...
function run(seed, cams) {
  ...
  staff(w, open, POLICE);
  staff(w, open, 3, "zoo");
  staff(w, open, 2, "centre");
  const placed = watch(w, cams);
  apply(w, { kind: "toggle", key: "noDisasters", value: false });
  const N = w.w * w.h;
  const seen = new Set();
  const seenArrest = new Set();
  const clearance = createClearanceTracker(KNOBS.CASE_MONTHS);
  const c = {
    opened: 0, arrests: 0, cold: 0, coldWrongful: 0, burglaries: 0, identified: 0,
  ...
  for (let t = 0; t < YEARS * 12; t++) {
    ...
    clearance.observe(w.events.files, w.events.arrests, w.tick - 1);
    ...
    for (const line of notices) {
      ...
      if (/was wrongly convicted/.test(line)) c.coldWrongful++;
    }
    let hot = 0, lots = 0;
    for (let i = 0; i < N; i++) {
  ...
  const cen = census(w);
  return {
    seed, cams, placed, police: cen.policeStations, prisons: cen.zoos, campers: w.citizens.filter(c=>c.home<0).length, P: cen.P,
    ...
    coldWrongful: c.coldWrongful,
    identified: c.identified,
    burglaries: c.burglaries,
    solved: c.opened ? 100 * clearance.count / c.opened : 0,
...
const FIELDS = ["police", "prisons", "campers", "P", "opened", "arrests", "wrongful", "exonerated", "cold", "coldWrongful", "identified", "burglaries", "solved", "sceneDark", "crime", "hot", "valveR", "watched", "approval"];
const rows = [];
...
if (CSV) {
  ...
  console.log("| cams | placed | pop | opened | arrests | solved | cold | wrongful | exonerated | cold, wrongful conviction |");
  console.log("|---|---|---|---|---|---|---|---|---|---|");
  for (const n of CAMS) console.log(`| ${n} | ${r1(placedAt(n))} | ${Math.round(mean(n, "P"))} | ${r1(mean(n, "opened"))} | ${r1(mean(n, "arrests"))} | ${r1(mean(n, "solved"))}% | ${r1(mean(n, "cold"))} | ${r1(mean(n, "wrongful"))} | ${r1(mean(n, "exonerated"))} | ${r1(mean(n, "coldWrongful"))} |`);
  console.log("");
  ...
  console.log("");
  console.log("Cameras have no direct crime-reduction term. Population, camping and mood changes can still change measured crime. Historical branch numbers use older custody/camping rules.");
}
