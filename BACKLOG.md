...
OWNER CALLS OPEN (§5): **Q3 IS ANSWERED BY A FRAME** (terracotta kept -- `docs/shots/scene.png` against its predecessor). **Q1 IS SHOT AND WAITING** — `docs/shots/sheet-shadows.png`, one town and one camera with only the knob moving: no shadows · k=0 (0.77% of the panel changed) · 0.25 (4.12%) · 0.55, the default (7.96%) · 1.2 (10.30%). Also open: dusk as a toggle or a clock; terracotta R roofs as the read-the-zoning-from-the-air move.
