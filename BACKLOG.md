...
**Generations and skills — PROPOSED 2026-09-07, awaiting the owner's answers:** [PROPOSAL-GENERATIONS-AND-SKILLS-2026-09-07.md](docs/PROPOSAL-GENERATIONS-AND-SKILLS-2026-09-07.md) — the owner's three rounds of rulings verbatim (§0), the measurements (§2), the design as ruled (§3), seven open questions (§5) and a build order (§7). Three passive instruments: `tools/tenureprobe.mjs` (tenure, bands, keeper succession), `tools/lineageprobe.mjs` (the ratchet as a shadow ledger), `tools/reachprobe.mjs` (walking tiles vs Chebyshev, rail free vs priced). THE FINDING: natives never breed — a cub's sixteen-year split makes a one-animal household and nothing merges households, so the L1 wedding's household-merge rule is step zero of the arc. Keystone handoff §36.
