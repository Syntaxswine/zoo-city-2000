...
**THE MEAT MARKET AS A PLACEABLE 3×3, AND FOUR LOOKS — asked 2026-09-24; M BUILT 2026-09-26 (`8cb6f35` instruments · `01aa0bb` the market · `e052aa9` the guard's month · `5f1aab4` the street trade · `991991e` the art · `05ce7eb` the retirement · the words — [the proposal's status](docs/PROPOSAL-MEAT-MARKET-2026-09-26.md) says what the build changed); L1–L4, Qz, Q1′ OPEN (the handoff is [HANDOFF-THE-MEAT-MARKET-AND-FOUR-LOOKS-2026-09-25.md](docs/HANDOFF-THE-MEAT-MARKET-AND-FOUR-LOOKS-2026-09-25.md)):** the owner — *"i want to change the meat market into a placeable 3x3 for light and high density. the models need to be redesigned. i'd like a few variations. so thats a coding and aesthetics change."* A SIM change, an art redesign and a save question, and it reverses the owner's own ruling: the crime-and-punishment design made meat a ZONE because *"a zone gets drag, undo, density, the tier ladder, decay, the WHY NOT line and the art audit for free; a placed civic gets none of them"*, and gave it its own valve because civic jobs count as shop jobs, and 48 meat jobs counted that way starved real shops. Today a hall is a zone and a tier (`isHall`, `meat.js`), about 25 modules test the zone for themselves, and no fixture in the tree has a meat lot. The owner answered five of the six questions on 2026-09-25: the zone goes; one tool, with H choosing Light or High; the market GROWS, and its sprites are the stages of growth (*"the reason for multiple sprites is to show the growth of the market"*); it costs what one meat tile cost to zone (§12); and old saves may break (*"the player base is basically me"*). The last two were answered on 2026-09-26: *"for light it should be 3-27 jobs / for heavy 27-180 / … 3-27 in stepped increments with different sprites"* (so Light grows too, which overrules my lean). In the same message the owner added a STREET TRADE: *"what if you didn't have the meat market and people were just trying to sell it on the street, this would turn the negative effects of the meat market into a walking hazard."* The design is written as [PROPOSAL-MEAT-MARKET-2026-09-26.md](docs/PROPOSAL-MEAT-MARKET-2026-09-26.md). It has seven stages on one ladder (a bare site; 3 / 9 / 18 / 27 jobs for Light; 27 / 72 / 180 for Heavy), each with its own sprite and smelling as the zone did for the same jobs. Where there is no market, or under prohibition, sellers work a pitch that moves every month. Each pitch carries a stall's dread and crime and the buyer's pull on killings, and exposes the prey that walk past; the mayor gets nothing from it. It waits on the owner's OK to its Part D. Beside it, four looks I proposed when asked, none ruled on: the one-box factories get a roofline (`sawtooth`, `stack` and `tank` are in `BLOCK_KIT` and no factory uses them), street lamps that come on at dusk (a head in `-` is free; a pool of light is a decision), shirts that say where an animal works (the owner's taste), an earth edge under the map. And two questions: which zoom the owner plays at (a 2× head is never seen at zoom 1, so it decides T4.1's place), and Q1 shot again on a dense block.

**THE NEWS MIX — WEDDINGS — found 2026-09-26, OPEN:** `tools/newsprobe.mjs` (an instrument, not a gate) holds people stories to 40% of the news. Since crime took its own section of the reader (2026-09-26) it reads them against the NEWS section, where they are 52.4 / 42.3 / 56.4 / 33.3% on seeds 7/3/5/11 over 30 years, and 69–77% in a policed town. The cause is the WEDDING story (`1e9d983`, 2026-09-07): 274 of seed 7's 808 rows, 794 of a policed town's 2,052. (An earlier version of this line blamed the meat market — one market making fewer meat headlines than eleven zoned halls. That was a guess, and the tally says weddings; the market only tipped seed 7 across the line.) Every named animal still resolves and survives save/load. With crime in its own section, weddings may be the next flood: the owner's call whether they get a section, a digest, or fewer.

**SPRITE UPGRADE — proposed, GREENLIT, TIERS 1, 2 AND 3 CLOSED, TIER 4 UNDER WAY (T4.0 AND T4.2 BUILT, T4.1 NEXT) — 2026-09-22/24 (sessions 21–22; the list is [PROPOSAL-SPRITE-UPGRADE-2026-09-22.md](docs/PROPOSAL-SPRITE-UPGRADE-2026-09-22.md), §4; the why and the traps are handoff §43 and §44; the STANDING BRIEF for whoever takes Tier 3 or Tier 4 is [HANDOFF-THE-SPRITES-2026-09-23.md](docs/HANDOFF-THE-SPRITES-2026-09-23.md) — the laws, the instruments, what is PROVEN against what is a GUESS, and the traps that are classes (eighteen by T4.2); nothing in the arc touches a sim rule, a saved byte or a knob):** the owner — *"how would you upgrade the sprites for animal city 2000?"* → *"you are greenlit to work on the problems you identified. start with documentation so you have a list to cross off as you go."* Diagnosed off a freshly regenerated `docs/shots/scene.png` (through `js/iso/painter.js`, so the picture judged is the picture played) and measured before anything was proposed: **nothing in the game casts a shadow** — nine `grep -i shadow` hits, every one a comment about *face* shading — and **66.0% of every standing pixel is a TOP face** (1,069,841 of 1,621,148 across 315 box recipes), the largest surface in the game carrying the least information. Four tiers: T1 the flatness (cast shadows from `RECIPES`, contact darkening, walker foot blobs, a dusk tint table) — **additive, no existing sprite changes**; T2 material vocabulary (four new ramps, zone-legible roofs, roof furniture); T3 window states, setbacks, ground variety; T4 authored species heads. The sequencing is set by the five ART GATES written out in §2c — the prism gate and the ink-identity gate together are why a cast shadow can never be a box in a recipe. Baseline green on `4bb38b6`: **970 checks, 0 failures**. **TIER 1 BUILT THE SAME DAY (T1.1–T1.4; `js/art/shadow.js`, the two union passes in `js/render.js`, `tools/check-shadows.mjs` 22 checks, `tools/shadow-sheet.mjs`):** the shadow of a box turned out to be a RECTANGLE — the shear runs along one axis and a box's a-range and c-range are independent intervals, so `a ∈ [a0 + k·c0, a1 + k·c1], b ∈ [b0, b1]` is the whole of it and the z-buffer unions them for free. 315 of 315 recipes cast; citizens, trees and tents get an ellipse that is NOT stamped into the pose (`allCitizens()` still 3236). Masks are unioned OPAQUE into a scratch canvas and blitted once per density, so two neighbouring shadows do not compound into a smear. **`art-dump` on the finished tree: 3,651 sprites and the palette, NONE MOVED** — the additive claim proven, not asserted; `setShadows(false)` returns the pre-shadow frame byte for byte. TWO PROPOSAL CLAIMS WERE WRONG AND THE PRE-REGISTERED CHECKS FOUND THEM: k=0 is the PLAN's ground footprint, not the tile diamond; and ink is NOT monotone in k — a roof slab slides its shadow off its own footprint, so area dips before it grows (`C3x3-emporium-1`: 8,855 → 8,772 → 9,863) while REACH is what cannot decrease. Mutation-tested 5/5 after the first round found two gaps in the gate itself (shadows-off-by-default survived; the key check compared the constant with itself). TRAPS: a shadow's bounding box need not contain the anchor (the security camera, fixed with a face-less `extent` box); at zoom ≥ 2 the renderer blits the HI-RES TWIN, so an ownership check that samples the 1× rows reads 176 false positives that are zero at zoom 1. T1.5 (dusk) split off — it carries Q2.

**TIER 2 BUILT 2026-09-22 (T2.1-T2.3; `js/art/roof-furniture.js`, three added ramps, `ROOF_OF`):** the roof says the ZONE now -- R terracotta, C light concrete, I rust, M dark slate -- routed through ONE table that reaches the six base families, `building-plans.js`'s `cap()` helper (already the choke point for all twenty-four authored variants) and `blocks.js`. Three ramps added (`tile` BCDE, `timber` LMNO, `fabric` PQRS) with `ADDED 12` keys and NO existing key or sprite moved; `furDark` deferred to T4 where its consumer lives. The recolour moved 84 sprites and changed the ink of NONE. Then roof furniture, FOUND rather than placed: `decksOf` returns the tops a recipe already exposes to the sky and lays rails, plant rooms, vents, tanks, extractor stacks and washing lines on them, so a family that changes shape keeps its furniture (the idea is `socketsFor`'s, widened from one point to every deck). TOP 66.1% -> 63.4%; the BARE-QUAD share -- added to faceprobe because a parapet's cap is itself a top face and the headline share cannot see this work -- goes 77% -> 56% on a store, 68% -> 44% on a shed, 55% -> 33% on a tower. The furniture CASTS ITS OWN SHADOWS with no shadow code touched (9,640 -> 9,748 px), the dividend for having put the shadow on the recipe. TRAP: railing every deck made CONCENTRIC BULLSEYES of the hipped R roofs (a stepped slope is four inset decks); a pitch step carries the next on ~77% of its area and a flat roof carries a plant box on a few per cent, so `coveredShare < 0.35` separates them with no family declaring itself. NEXT: T1.5 dusk (carries Q2), then Tier 3 (window states, setbacks, ground) and Tier 4 (authored species heads). Handoff §43 carries the ten traps, including the three that cost the most: a shadow's bounding box need not contain its anchor; at zoom >= 2 the renderer blits the HI-RES TWIN, so an ownership check on the 1x rows reads 176 false positives that are ZERO at zoom 1; and an exit code read through a pipe is the PIPE's, which reported a real suite failure as green for a round.

**TIER 1 CLOSED 2026-09-23 (T1.5, dusk; `js/art/dusk.js`, `tools/check-dusk.mjs` 113 checks, `tools/dusk-sheet.mjs`):** the evening is ONE key -> key table through the same `rasterize(rows, tint)` door a species skin and the water's cycle already use, so every sprite in the game goes dark at once and not one is re-authored -- **`docs/fixtures/art-baseline.txt` does not appear in the diff**, 3,651 sprites and 74 keys, `TOTAL 86cc0399`, byte for byte what Tier 2 left. THE MECHANISM: `litSkin` has always given a ramp's BRIGHT rung to the top face and its DARK rung to the end, so **the ramp index already says which way a surface points** -- warm the bright rungs toward a low sun, cool the dark ones toward the sky, and a table with no geometry in it lights the city from a direction. Each key's evening colour is then projected onto the nearest key the palette already has (found, not authored), with a 60% toll on leaving its own ramp so terracotta stays terracotta (`BCDE -> 1BCD`; 36 of 64 stay home). The lights and the marks are fixed points: a lamp does not dim because the sun went down, and the zone chalk was hand-tuned once already to not vanish into the lawn. FOUR CLAIMS BROKE ON THE PALETTE: (1) "a rung cooler" is half of it -- the floor is PINNED by the shadow key's near-black claim, so an evening taking its darkness out of VALUE runs all fifteen ramps' dark rungs onto FOUR keys; the dim is a fifth, the cool is two thirds, and the rest comes from the shadows (dusk MULTIPLIES `SHADOW_K` x3.2, so Q1 still owns the length); (2) a `night` ramp built for that was **taken out again** -- worth 0.1 of a luminance point and the same four rung-0 destinations either way, because what starves the dark end is the floor, not the hues at it; (3) "the ground is never brighter than the same key standing" is false of the TRANSFORM -- the sky has a luminance of its own (65.3) and a key darker than it gains by seeing more of it; (4) the monotone floor never fired once over 6,000 adjacent pairs and a mutant deleted it with the suite green, so it is gone. The GROUND PLANE is the one facing the renderer knows for certain, and it takes its own deeper table (mean -30 without, -45 with). **Q2 IS ANSWERED BY THE SIM:** a tick here is a MONTH, so the only clock available would make dusk a SEASON -- it ships as a mode, an Options checkbox beside the cheat and `/`, as THIS BROWSER'S preference, because a city sent to someone else must not arrive at nightfall. And the knob is NOT SMOOTH (5 of 64 keys move at 0.1, 67 at 1.0; between 0.4 and 0.7 only 11-20 stay in their own ramp), which is why one setting ships and the sheet shows the middle on purpose. Mutation-tested 9/10 after the first round found four gaps, one of them the class that survived `check-shadows` too: **a check that iterates the module's own list moves with the code.**

**TIER 3 OPENED 2026-09-23 (T3.1, window states; `js/art/building-character.js`, `tools/window-sheet.mjs`, Part E 8 → 18 assertions):** a window was a light switch -- glass, or the lit key -- and `characterSprite` now returns what the pane HOLDS: a drawn blind, a plant in the window, a boarded pane on a building old enough to have lost one. Still ONE return in ONE face function, on the same 2x3 grid of world-unit cells the lights have used since People E, and **`art-dump` is unmoved: 3,651 sprites, 74 keys, `TOTAL 86cc0399`** -- everything a pane can hold was already in the palette (fabric for the blind, timber and slate's dark rung for the boards, canopy for the leaves). THE APERTURE MEASUREMENT CAME FIRST AND SHAPED EVERY STATE: over the 192 glazed plans there are **17,473 window cells carrying glass and a cell is on MEDIAN only a THIRD glass** (2.0 of 6 world units²) -- 15.8% hold glass in the bottom third only, 12.2% in the top third only -- so a state painting a fixed sub-rectangle of a CELL paints nothing on a sixth of the city and covers everything on another eighth. A state is therefore whole-cell (blind, board) or FOUND FROM THE APERTURE by probing the skin underneath, which is `architecture-detail.js`'s own `edgeV` move. **THE FIXTURE HASH WAS A FUNCTION OF THE LIGHTING HASH AND SEVEN CHECKS WERE GREEN OVER IT:** the lights use `(3cu + ck + phase) & 3`, the fixtures were given `(5cu + 11ck + 7phase + 3) & 15` on the reasoning that (5,11) is not a multiple of (3,1) -- but **5 ≡ 1 and 11 ≡ 3 mod 4**, making the second form exactly `3·cell + 3 mod 4`, so cell class 1 could never carry a blind, classes 2 and 3 never a plant, and the backlit cloth on a facade did not move when the building filled up (11,690 px at lit 1 AND at lit 2). Any linear form mod 2^n reduces to a linear form mod 2^m; both hashes are the terrain's avalanche now, and **an EXISTENCE check cannot see a lock** -- the three that replace it demand a number that MOVES. TWO THINGS WERE MEASURED OUT: the plant's POT (14 px of 54 at 4x, nearly all of it under the detail pass's sill, and at 1x it inverted so a window plant read as a BROWN pixel -- the same move as the evening's `night` ramp) and the plant's one-way probe (the owl landmark's perch windows are 0.8 units tall and a 1.4-unit band swallowed them whole). The detail pass had to be TOLD what an aperture is: a character box now carries `aperture()` saying where the BARE skin's glass ran, or a blinded pane reads as wall at 2x and 4x and loses its frame.

**T3.2 BUILT 2026-09-23 (setbacks and awnings; `js/art/buildings.js`, `tools/massprobe.mjs`, `tools/skyline-sheet.mjs`):** MEASURED FIRST, and it re-read D3. `massprobe` (passive: `fill` = a plan's rooms over their bounding prism, `deck` = a level between the street and the top, at zoom 1) found "R, C and I are the same prism" had become true of FOUR PLANS: session 1's two-storey, apartment, store and tower were the only R/C tier-2/3 plans that were one box with a lid (fill 1.00, deck 0.0–5.3%), drawn by a third of those lots. They step back now — a terracotta porch roof round a set-back upper storey; an 8×8 front-corner terrace with a balustrade and a planter; a glass shop floor under a canvas awning with the storeys set back; a tower on a lobby podium — on BOTH street faces, because `flipPlan` mirrors boxes and not skins. `art-dump`: eight sprites moved, the palette line did not, `TOTAL f910d9fe`; city-wide one-box plans 51 → 43. TRAPS: an awning d deep hides 2d units of wall under it — the first canopies took the store's door 36 → 15 px and the tower's 36 → 9, and the pre-registered door count caught it — and a gap left for a door must allow for the SHEAR; `fill` counted the slab between two storeys as void and flattered the apartment to 0.848 (0.875 honestly — the metric was fixed and the terrace grown, not the bar moved); RECIPES grows with every lit building drawn, so the probe reads PLANS. The roof furniture cannot tell a terrace from a pitch step (median coverage 0.63 vs 0.65), so the balustrade is drawn into the plan. Two fixtures T3.2 exposed went first, each alone: check-dusk's "open lawn" samples were a wall, the river and a tree (`b39866c`), and two sheets had been stale since T3.1 (`c61920c`). Verified on the DEPLOYED Pages build, the preview slots all being held by other chats: four hashes as art-dump says, a 218-lot block in a real browser, no console messages. Handoff §46; the standing brief carries the four things to know before massing a plan.

**TIER 3 CLOSED 2026-09-23 (T3.3, ground; `js/meadow.js`, `js/art/terrain.js`, `js/render.js`, `tools/check-ground.mjs` 51 checks, `tools/groundprobe.mjs`, `tools/ground-sheet.mjs`):** MEASURED FIRST, and the list's diagnosis was half right: the eye was counting not a REPEAT but a QUILT — the three grasses are three brightnesses (tile means 117.5 / 122.8 / 112.6 luma) and each was exactly one tile, so a field was a patchwork of diamonds. `groundprobe` reads it off the renderer's frame: the step across a tile edge over the step across a line through a tile was 2.46 / 3.80 / 5.13 at zoom 1/2/4. THE GRASS IS KEYED OFF ITS CORNERS NOW (`692b2b3`): a level (kept · meadow · rough — which ARE grass-0/1/2) on every tile vertex, a tile drawing its four corners interpolated, so no edge can step — **0.96 / 0.99 / 1.01**, against a seamless control's 0.96 / 0.99 / 1.16; the repeat 13.6 → 4.9 points. Kept wherever the city has made something (so every road and chalk margin meets its own grass), meadow and rough elsewhere from the world's saved tile bytes; 31 corner combinations × six dithers, 183 sprites added, none moved, no colour. WORN PATHS (`a416748`): I first wrote that no walker crosses grass, read off `walkers.js` — but a commuter walks the path the SIM stored, and `fields.js` lays a station's FORECOURT into it tile by tile, so `wornPaths` reads those steps off the stored paths and `art.footpath` wears an earth track exactly where they run (30 sprites; trodden below four walks, worn from four; no scripted town wears one, a player's set-back station one or two tiles an end). BOTH PRE-REGISTERED BARS INDICTED SOMETHING: the seam bar its own METRIC (1.71 on a control that cannot have a seam — one fixed pair of strips read six dither patterns) and the repeat bar the DESIGN (four dithers 6.4 against ≤ 6; six read 4.9). The gate checks the frame pixel for pixel — every open tile is the sprite its own corners, byte and walks name — and 24 of 24 mutants die; the suite's own twin gate refused the first paths (a thin trodden path drawn per pixel was another path at 2×, 19% light), so which pixels are earth is decided world-sized now. First, alone (`e2ace35`): `building-variants/` had been stale since T1's shadows and `scene.png` since T3.2, because no refresh ran the tools that write them. Handoff §47; the standing brief carries the five things to know before touching the ground, and two new traps (14: a claim about a consumer is a claim about its producer; 15: a metric needs a control before it gets a bar).

**TIER 4 BEGUN 2026-09-23 — T4.0, THE COATS (`b72bea7` · `a2dc8ea` · `bfb93e8`; `js/art/citizens.js` `COATS`, `js/art/citizen-detail.js`, `js/ui.js`, `tools/check-animals.mjs` 15 checks, `tools/zooprobe.mjs`, `tools/zoo-sheet.mjs`):** the standing brief said to start from the coats, and they were worse than it said. `zooprobe` pulls each animal apart into its FORM (the figure in one coat, against one animal mid-stride) and its COAT (against one animal shaded): **7 coats for 14 species** (not 8 — the tortoise wore the fox's), the **beaver and the bear ONE ANIMAL**, **five dark coats FLAT** in their shaded look (one rung down a four-rung ramp, the lit and the shaded body land on one key — 18% of the kit's citizen sprites), and the **dark greys 4.0 from the road**, closer than the olive tortoise that "vanished into the lawn" (9.8) ever was. The coat table moved from the sim's roster into the kit, byte for byte, then got fourteen coats from ramps that existed — red fox, coral pig, ginger cat, brown bear, tawny owl, rufous hawk, warm greys because the cool ones are the road's — with **no key added**: `furDark` was built, its keys fell within ΔE 4.2 of existing ones and it re-mapped seven keys of the evening, so it came out. 2,774 sprites moved, all animals, none in size, anchor or ink. The gate reads the pre-T4 table in the same run as a control that must fail exactly as measured; 15/15 fault mutants by aim, one neutral that passes. On the way the 2× pass was found to have NEVER reworked the hawk's fur (0.0% — it found fur by ramp, the hawk is `earth`) nor the tortoise's (2.4% — its outline meant no edge ever fired); it asks the composer now. Handoff §48; standing brief traps 16 (a clamped shift puts two rungs on one key — three times in one item) and 17 (a mutant killed by the wrong check proves nothing; a survivor may not be a fault).

**T4.2 BUILT 2026-09-24 — THE BUILDS (`01cf866` · `4ea9e5e`; `js/art/citizens.js` `BUILDS` and the `stout` body, `tools/zooprobe.mjs` `zooReadings({ builds })`, `tools/check-animals.mjs` 21 checks):** the big build carried four figures the eye could not split without the fur — the bear and the pig 3.5 apart (FORM, the coat taken away), the beaver and the bear 4.0, against a bear mid-stride 5.7 from itself — and the beaver and the pig stood 1.08 strides apart in coats too alike for the eye to split. **One new body cannot part three animals that must each stand apart**, measured across every assignment of the six in a lab worktree; the tables that brought all four to the small build's own closest pair put the wolf or the pig on the slim body, and a slim pig is not a pig. So the PIG wears a third build, `stout` — a barrel on thin legs, the belly out on the side it faces, the back flat so its tail still shows — and the BEAVER wears the small one, where its paddle shows facing SE for the first time. bear/pig 3.5 → 10.4, beaver/bear 4.0 → 14.0, beaver/pig 5.9 → 14.6; bear/wolf stays 6.7 (1.18 strides), their coats twice the shade floor apart. The instrument went first, no sprite moving: the build table exported and an instrument's hook (`opts.build`) so the old bodies are drawn by the kit's own composer — T4.0's control is read on the bodies it was measured on and T4.2's in the same run, rather than their expected numbers being "changed deliberately". The gate: no two figures closer than a stride; where the coat cannot split two animals the figure must, 1.4 strides (the rabbit and the mouse, 1.48, the witness); only the beaver and the pig moved. 256 sprites moved, none in size or anchor, no cub, portrait or colour. On the way T4.0's own check that FORM ignores the coat was found comparing two readings of the same sprites — green whatever FORM was made of — and replaced by the property (standing brief trap 18). Handoff §49.

OWNER CALLS OPEN (§5): **Q2 IS ANSWERED BY THE SIM** (a mode, not a clock -- there is no hour in this game). **Q3 IS ANSWERED BY A FRAME** (terracotta kept -- `docs/shots/scene.png` against its predecessor). **Q1 IS SHOT AND WAITING** — `docs/shots/sheet-shadows.png`, one town and one camera with only the knob moving: no shadows · k=0 (0.77% of the panel changed) · 0.25 (4.12%) · 0.55, the default (7.96%) · 1.2 (10.30%). NEXT: **T4.1, authored 2× heads** (ask first which zoom the owner plays at — a 2× head is never seen at zoom 1; Qz in the meat-market handoff) — on a shared build two animals are only their heads and their tails (handoff §49), so the head carries more than it did; a twin may not expand the 1× silhouette (`check-closeups`) and `CITIZEN_DETAILS.authored` says what the composer drew and where. OPEN from T4.2: the bear and the wolf share the big build 1.18 strides apart (their coats split them, 35.1 against a shade floor of 16.6, and the one-coat rule refuses the day a coat change closes that); cubs share one body and zooprobe has never read them; the stout's shape and who moved are taste inside two rules. The whole ledger, done and unfinished, is the first section of the standing brief. OPEN from T4.0: `species.js` still carries `fur`/`furShift`, read by nothing but check-animals' control — a sim-side commit should remove them, and must rewrite that control's premise check with them (it asserts those columns ARE the pre-T4 table); the beaver's 2× rule reads `ramp === "earth"` and has never fired on a beaver without a sack; a coat at the top of its ramp gets less edge light at 2× (the wolf 18.2% → 10.0% of its fur reworked, inside the gate); the coats are taste inside four rules and nobody has watched them in play. OPEN from T3.3: scatter as OBJECTS (flowers, stones) was not built, since at zoom 1 each is a pixel of a non-grass key and every such key on the ground already means something; the meadow's numbers are taste nobody has tuned in play; a path is worn on open grass only (a forecourt across chalk, a park or rubble shows none). OPEN from T3.2: the roof furniture's terrace question (a class change for `roof-furniture.js`, numbers in handoff §46); NO GATE SEES A HIDDEN DOOR (twice now — the meat hall's annex and T3.2's canopy draft); 43 plans are still one box — the industrial sheds and works, the tier-1 shops and cottages, the meat stalls (`node tools/massprobe.mjs` names them; L1 in the meat-market handoff takes the industry first, and its item M redraws the meat). Whoever takes them should know the cemetery is still 88% roof and 74% of that one quad.

**Campaign targets RE-SET 100 → 400 → 800 → 1,200 — 2026-09-14 (session 20; `docs/CAMPAIGN.md` Chapters; the one table is `CHAPTERS` in `js/sim/progression.js`, and the Rules tab, the new-city form and both fixture files now READ it):** the owner on the review's first open decision: *"thats a good point, lets lower the target. but that suggests implicitly that the later targets also need to be lowered"* → *"100 → 400 → 800 → 1,200 sounds right, lets try it out."* Measured before the change (scratch `campaign-ceiling.mjs` = the calibrated free-play mayor on a campaign map with farms kept ahead and the chapter LOCKS bypassed — the ceiling; the review's weak Low-density player — the floor; neutral taxes, disasters on): the floor reaches 400 in 1.8–2.8 y and 800 in 4.9–16.3 y on two of three seeds (seed 3 sinks into receivership from year 5 at ≈ 530); the ceiling reaches 400 in 3.0–3.8 y, 800 in 4.3–4.8 y and 1,200 in 6.1–9.0 y, then plateaus at 1,400–1,500 and never saw 3,000 on any of five seeds (1,500 took 104–373 months). The economy is untouched — a Low-density town still runs a deficit past a few hundred animals (`UPKEEP_CITIZEN` §12 vs Low R tax); the Metropolis is earned by fixing that. OPEN: the Chapter 5 sanitation goal (90% coverage, backlog ≤ 25%) has not been played through by either scripted player (neither builds works); a play-through that reaches the Metropolis is owed.

**Hostile review of the campaign/healthcare/governance commits — 2026-09-13 (session 20; `docs/REVIEW-HOSTILE-2026-09-13.md`; `tools/check-hostile-review.mjs`, 17 blocks, 19/19 reverting mutants caught):** nine commits by another agent (`a925df5..7b59fbb`) reviewed under three lenses. FIXED: the Butchers' licence and the Scrubbers restored for a town with no Governor (striking them had re-rolled every free-play event stream past 15 I lots; the licence is owner-ruled, SPEC §9c); `governanceUnlocked` DERIVED, not a stored bit (undo/bulldoze of the mansion left the town governed and un-hashable); food aid no longer passes a chapter (`infrastructure.farmFood` is the gate); the Chapter 1 gate reads the TARGET, not the head-count (four farms held only at exactly 100; 22–38 months → 10); governance ops pay `canSpend` (receivership bought scrubbers); a flags-less save loads; trees/walls/Use open in Chapter 1; a locked palette click flashes its reason; README/SPEC/flash/undo copy. THE FALSIFIER: `NO_MEDICAL_PENALTY=0` on the suite's scripted city returns the pre-merge `366 / e0833adf` EXACTLY, pending licence card included — the nine commits plus the fixes are byte-identical to `a925df5` when the one new free-play rule is neutral. KEPT: the −3% no-doctor lifespan rule (measured a chaotic perturbation: balanced −2%, estate +8% the other way at 60 y; the canary's 366 → 271 is the cascade) under accuracy-over-determinism. OWNER DECISIONS OPEN: (1) Chapter 3's 1,500 is unreachable on the current economy — six 40-y scripted runs peaked at 892–934 and entered receivership in years 6–10 (`UPKEEP_CITIZEN` §12/animal vs Low R tax); lower the target, scale campaign upkeep, or a campaign tax curve; (2) the remote — silver bevelled shell, 29 pictograms with NO visible label or key, ~840 px tall, against SPEC §11's sprite thumbnails and the field-guide aesthetic; (3) `[`/`]` are AltGr chords on European layouts and `input.js` drops altKey; `C` is Cemetery beside Commercial = 2; (4) the check.mjs over-capacity audit was widened to skip `absent` citizens because a prisoner keeps a decayed home's address — pin the sim state instead; (5) `sanitationShare` counts campers in the denominator (>10% campers can never clear 0.9).

**Parks have no workers, 2026-09-07:** the Large Park's twelve jobs are struck. The owner: *"you are correct to strike them, parks have no workers. i think the workers might have been erroneous leftovers from when the large park was still called a zoo."* `civicJobs` lists no Large Park, so the C valve no longer counts a park as a job site; a city saved while a park employed lets those hands go at load, silently (`save.releaseOrphanJobs`, no LOST_JOB chapter); the card no longer prints a park's jobs. Two regressions in `tools/check-civic-campuses.mjs`. The mayor's rigs are byte-identical (she builds only 1×1 parks).

**Generations and skills — STEP 0 BUILT 2026-09-07 (weddings, companions, the twelve temperaments, the full-home litter; SPEC §7.2, §7.5, §7.11; `tools/check-generations.mjs`; every rig hash moved, balanced `c055aba5` → `12427ef4`, estate `2ced10f8` → `747d57d3`, neutral knobs give `c055aba5` back) — the rest PROPOSED, awaiting the owner's answers:** [PROPOSAL-GENERATIONS-AND-SKILLS-2026-09-07.md](docs/PROPOSAL-GENERATIONS-AND-SKILLS-2026-09-07.md) — the owner's five rounds of rulings verbatim (§0), the measurements (§2), the design as ruled (§3), eleven open questions (§5) and a build order (§7). Five passive instruments: `tools/breedprobe.mjs` (three household-merge shapes run as a pass OUTSIDE the sim; the population ledger by decade), `tools/commuteprobe.mjs` (commute vs the benefit radius and the species' preference; the jobless' nearest open job), `tools/tenureprobe.mjs` (tenure, bands, keeper succession), `tools/lineageprobe.mjs` (the ratchet as a shadow ledger), `tools/reachprobe.mjs` (walking tiles vs Chebyshev, rail free vs priced). THE FINDING: natives never breed — a cub's sixteen-year split makes a one-animal household and nothing merges households, so the L1 wedding's household-merge rule is step zero of the arc. ROUND 5 (the same day, later): the owner wants levers for migration AND growth — *"if there is no breeding then pacification is mostly an empty threat"* — and rules that crews may commute beyond their service's benefit range. §8 measures three merge-rule shapes (births +45–55%, the town 53–65% town-born at sixty years and 84% at ninety, generation 4–5; population itself moves −20% to +2%, because size is set by jobs and homes and the rule sets WHO fills them; the pairing rule decides whether a quarter of households are mixed, seven in a hundred, or none — q1). §9 shows commute is already decoupled from the benefit radius (50–100% of every crew lives beyond it; nobody sits jobless beyond 40 steps of an open job on any rig, so the demand stretch would hire nobody there — q9). SPEC §7.2 corrected: the ×0.25 full-home litter was never wired (`BIRTH_FULL_MULT` is dead — q11) and the "move-in at 16" never existed. ROUND 7 (evening): *"migration should definitely be bidirectional. it should take more than just a fire for people to leave, it would have to be a combination of factors"* — `tools/leaveprobe.mjs` reads every departure's cause from the archive: today every one is FRICTION (a friendless house), a fire alone moved nobody on the rigs (but the homeless path removes a family with no tent tried), and the town closes at ninety years (13 in, 30 out a decade); the PUSH shadow — nine weighted factors, score ≥ 3 leaves at 0.05·(score − 2) a month — gives 1,151 departures and 3,188 arrivals per sixty years (268 out, 37 in a decade at ninety), threshold 2 is a churn machine and 4 is lumpy; the estate drains without roots (§10, q12–q14). RULED the same night: threshold 3, roots by years at home and a town-born adult, the nine factors — *"written plan only, we are about to compact after its scoped"*: **THE PUSH BUILT** later that night in two commits (`7e701e0` the fields and the tent, rigs unmoved but for the two saved fields; then the push in place of friction): nine grievances, threshold 3, `LEAVE_P` 0.05, roots by years at THIS home and a town-born adult, the MOVED AWAY line naming the reasons, card C5, the departure record carrying the score as decided, `tools/check-migration.mjs` (22 checks; 7 + 15 mutants, all caught), the falsifier `--set LEAVE_P=0` → `52fde3b4` / `c3843687` holding. ONE KNOB MOVED FROM THE SCOPE: `LEAVE_ROOTS_YEARS` 10 → 3 — at 10 the estate drained to 56–74% of its control on four seeds, at 3 it keeps 1,326–1,607 against controls of 870–1,691 (the estate rig is chaotic under disasters; with no door out it can seize and wither to 870). Balanced loses 274–693 per sixty years at 3 (the scoped 800–1,400 was calibrated on the rootless shadow). Nobody left under three as decided (§10e). NEXT: steps 1–4 (§7). Keystone handoff §36–40.
...
## Knowledge and culture — SHIPPED 2026-09-05 (session 17; SPEC §9e; handoff §31)

- [x] **University (3×3), Amphitheater (3×3), Library (2×2), Gallery (2×2)** — built as [proposed](docs/PROPOSAL-KNOWLEDGE-CULTURE-2026-09-05.md) with the owner's ruling on culture (mood AND land value) and the review's corrections; the proposal carries a BUILT note listing where the built thing differs. Keys K Y M T. `reach.floodBudget` is the half-map / eighth-map flood (whole layers nearest first, the last in tile order); `fields.computeKnowledgeCulture` the two per-tile fields; `+600·K` in the capacity law; CULTURE in mood and `LV_CULTURE` in land value; two overlays; the campus card with its reach on THIS map; Census and Rules K1; `tools/knowprobe.mjs` the dynamic. 852 checks (52 in `tools/check-knowledge-culture.mjs`), 10/10 mutants, browser-verified at both zooms, six mayor rigs byte-identical (the mayor builds none).
- [ ] **Open after the build:** the University's +600·K against the Large Park's +500 for less upkeep (the owner's call); a range preview under the placement ghost (the overlay on O shows the catchment the moment the building lands); the K-mean dynamic on the owner's control city; the school/accelerated-adulthood idea stays deferred.
...
## Wealth and class — RERULED 2026-09-06: a CHECKLIST, not points — "the affluent house MUST be in range of all of these" (session 18; SPEC §9f; handoff §35)

- [x] **The list.** The owner, on the points ladder: *"so it should not be about points in the sense that each item has a variable amount of influence, its more about a checklist, the affluent house MUST be in range of all of these, amphitheater, university, library, gallery, large park within 5 tiles, police, fire. and a shop within 10 road tiles"*. A class is a checklist now, every item required and none weighed (`KNOBS.CLASS_NEEDS`, `wealth.ITEMS`): the AFFLUENT list is the owner's word for word — an Amphitheater, a University, a Library and a Gallery each reaching the address (`world.civicReach`, a bit per building painted beside the knowledge and culture fields, which keep only the stronger), a Large Park within 5, a police station's cover, a fire station's cover, a shop within 10 road tiles; the MODEST list is the proposal's default, culture in reach and a park within 5 (unruled). Nothing else is on a list — not the air, the crime, the land value or the trees; the police station on the list is what a dense block's crime answers to. The card says which items are in range ("in range of an Amphitheater, a Gallery, police — 3 of 8 the affluent need") and which the next class wants. 68 checks in `tools/check-wealth.mjs` (922 in all), 15/15 mutants, six mayor rigs byte-identical.
- [x] **Measured (`tools/wealthprobe.mjs`, the graft is the list itself at §5,350/yr):** into the High block at year 0, the mansion in month 25 — the shop the last item for 22 months — the Russets (5 foxes) keeping the house and 149 moved out, none camping a year on; carved into the FULL block at year 5 (`--at 5 --clear`), month 66, 158 moved out; the planned quarter month 28; the control (`--bare`, four buildings and a park) never; the list's upkeep is the town's whole surplus — §1,444 at year 30 with TAX_CLASS [1,2,5] against −§46,983 with no class tax, so the progressive tax is what pays for it; justice, four seeds: mansions on 3 of 4, two affluent files (killings), both cleared, both the hall, a mansion never a hot lot (crime ≤ 11 under the cover on the list). Under the points ladder the same graft had raised mansions in months 10 and 27; under the gates never.
- [ ] **Open for the owner:** the MODEST list (culture + a park within 5 is the proposal's default); "in range" for police and fire read as their cover halos (a radius is one item away if tiles were meant); MANSION_P 0.25 against the shop that binds for two years; a mansion rises on EMPTY chalk the month the last item lands.

## Wealth and class — RERULED 2026-09-05 (night): "mansions should rise in dense blocks too. the biggest factor should be what amenities are near it" (session 18; SPEC §9f; handoff §34)

- [x] **The ladder is POINTS, not gates.** Culture at home (a Gallery 2, an Amphitheater 4), knowledge (a Library 1, a University 2), a park within 4 (a Park 1, a Large Park 2), a shop within 6 road tiles (1), water or trees beside (1) — ten in all; the street drags a point each for smoke (pollution > 40), a hot street (crime > 60) and a meat hall's dread, never more than three. MODEST at 3, AFFLUENT at 7 and culture at home (the owner's morning PREREQUISITE). Land value is no longer a rung (it is the tax's). The window admits whole blocks lying inside it — a 2×2 within, or the 3×3 block that IS it: the apartment block becomes the mansion. `world.klass`, the card ("7 of 10 points — an Amphitheater, a Library, a Park, trees or water beside; 1 off for the streets (crime 100)"; "2 more points: a University's knowledge (+1 more) · …"), Rules W1 and the MANSION line all say the points. 68 checks in `tools/check-wealth.mjs` (922 in all), 16/16 mutants, six mayor rigs byte-identical (without culture or a park an address has two points at most, and nobody leaves poverty).
- [x] **Measured (`tools/wealthprobe.mjs --dense`; `--at 5 --clear --university`; `--tax`):** the bare amenities at a High block's corner raise the mansion in MONTH 1 on empty chalk (7 points; under the gates: never in thirty years); the same graft carved into the FULL block at year 5 reads 6 of 10 for 300 months (no tile left beside it for a tree, the streets −1) and with a University raises THREE mansions in months 61–75 — 95, 151 and 173 animals moved out, 26 → 61 households in tents a year on, the town down from 677 to 436 by year 30 (293 affluent carrying 86% of the R tax); one Amphitheater reaches an eighth of the map, so its town is MODEST almost to the last address and pays ×2 — cash §203,294 at year 30 against §5,839 in the plain rig, −§338 with no class tax and §122,806 at ×1.5; the planned quarter's mansion came in month 12 (was 26); justice, four seeds: 15 files from affluent addresses in 120 town-years (was 1), every one cleared, 8 to the centre and 8 to the hall — the priority is real in the street now.
- [x] **Open for the owner (SUPERSEDED the next morning by the checklist above):** TAX_CLASS modest ×2 against a town-wide modest class; MANSION_P 0.25 against three mansions in fourteen months; a mansion on EMPTY chalk — the checklist answers the first (modest is culture and a park, not an Amphitheater's shadow) and the second (the shop binds for two years); the third stands.

## Wealth and class — SHIPPED 2026-09-05, REBUILT the same evening on the owner's rulings (session 18; SPEC §9f; handoff §32–§33)

- [x] **The owner's eight answers** (verbatim in the proposal's rulings note): *poverty, modest, affluent* · *class based on the opportunities near you* · *if there are enough positive things near you it happens naturally like when the building upgrades to an apartment building* · *less people are homed in the same area* · *the option for a progressive tax is a wonderful idea* · (the rungs: no ruling; the defaults stand) · *i like the art you picked, at some point a species specific variation is welcome* · *this applies just to the home, yes harsher punishment*. Rebuilt: class is what the ADDRESS affords this month (`world.klass`, derived by `wealth.computeClass`, never saved — no household field, no inheritance); a 3×3 is read at its HEART with nature round its border (`siteOf`); the Estate tool and plot are GONE — a mansion RISES from any 3×3 of housing whose site is affluent, at `MANSION_P` 0.25·score a month through `lots.lotScore` (`wealth.mansionWindow`, `raiseMansion`: the largest household that fits keeps the house, everyone else is moved out by `citizens.displaceFrom`); a mansion that comes apart becomes nine cottages; `TAX_CLASS` [1, 2, 5] as the progressive tax; justice by the class at the address. 911 checks (57 in `tools/check-wealth.mjs`), 18/18 mutants, six mayor rigs byte-identical, browser-verified. Art `js/art/mansion.js` (the plot sprite went with the tool).
- [x] **Measured (`tools/wealthprobe.mjs`):** the bare amenities at a High block's corner raise NO mansion in thirty years — the heart of any 3×3 inside a High block reads crime 100 (density is crime, the game's own law) and pig pollution; the quarter a player would PLAN (low density within two, a Large Park, police, one corner shop, Library and Gallery within five of the heart) raised one in month 26 on the estate layout (shops bound 19 months; the Scrapleys, 4 raccoons, kept the house and 26 animals were moved out), at §2,250/yr of upkeep that put a town of ~700 in the red for a decade; balanced never (the quarter does not fit); no file from an affluent address in 120 town-years of the justice run — the priority is real in the roll and rare in the street.
- [ ] **Open after the rebuild:** MANSION_P 0.25 against the months a planned quarter waits; TAX_CLASS [1, 2, 5] (a progressive-tax Options switch is one line if wanted); (a mansion inside a dense block — RULED that night and rebuilt: the section above); the species-skinned mansion set the owner welcomes; the centre bottleneck from the first build stands whenever an affluent file exists; the affluent "kept" look, shops as victims and a class weight in the C valve are not built.
- [ ] **Found in passing and FIXED:** the campsite guard in `ops.costOf` and the ghost footprint in `input.hoverForRenderer` both stopped at the centre — a Library could land on an occupied tent, and a Library's placement ghost was a 1×1 diamond; both read `CIVIC_SIDE` now, and a check walks every click tool's ghost against its footprint.
...
- ~~**The abattoir** — a 3×3 M landmark on the landmarks proposal's merge rule.~~
  RETIRED WITH THE ZONE (2026-09-26): meat is a placed market that grows
  through its own seven stages; there is no M block to merge into a landmark.
- **Per-species fixed counts** on the census histogram (`wolf 129 · 11 fixed`).
- ~~**The scripted suite city loses its M row to a year-7 fire**~~ OBSOLETE
  (2026-09-26): the scripted city has no M row. Its Light market is placed at
  t = 0 and asserted, and the dread invariant reads a market's stall tiles.
...
- ~~**Wedding**~~ BUILT 2026-09-07 as `citizens.weddings` (SPEC §7.2): a single
  courts within 12 road tiles, the same species preferred, predator and prey at
  one in ten, companions at one in ten, the twelve temperaments weighing the
  choice; the two befriend; story.js writes and flashes the WEDDING line. Still
  owed from the old note: the PROCESSION WALKER (the walker layer has no
  wedding walk yet).
