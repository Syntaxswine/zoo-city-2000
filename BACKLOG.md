...
OWNER CALLS OPEN (§5): **Q2 IS ANSWERED BY THE SIM** (a mode, not a clock -- there is no hour in this game). **Q3 IS ANSWERED BY A FRAME** (terracotta kept -- `docs/shots/scene.png` against its predecessor). **Q1 IS SHOT AND WAITING** — `docs/shots/sheet-shadows.png`, one town and one camera with only the knob moving: no shadows · k=0 (0.77% of the panel changed) · 0.25 (4.12%) · 0.55, the default (7.96%) · 1.2 (10.30%). NEXT: **T3.2** (setbacks and awnings on the R/C mid-tiers — the EMPORIUM in `blocks.js` already shows the pattern, not the tower), **T3.3** (ground variety; note the ground now has a second reader, `duskTable(amount, true)`) and **Tier 4** (authored 2× species heads, a third body build). Whoever takes them should know the cemetery is still 88% roof and 74% of that one quad, and that fourteen species wear only EIGHT coats.
