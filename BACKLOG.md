...
## Wealth and class — SHIPPED 2026-09-05 (session 18; SPEC §9f; handoff §32)

- [x] **The ultrawealthy, their mansions, and whose burglary gets worked first** — built as [proposed](docs/PROPOSAL-WEALTH-AND-CLASS-2026-09-05.md) on the proposal's own defaults (its eight §8 decisions taken and said out loud in the BUILT note): class on the HOUSEHOLD by the lot at arrival, inherited, kept (`hh.wealth`, omitted when 0); the ladder as GATES (`wealth.attainableClass` with the unmet list the card prints); R tax × [1, 2, 5]; the Estate plot (Q, §200) as an R block anchor with `world.estate`, chalk until every rung is met, then a mansion of capacity 8 for ONE household, no roll; fire and the bulldozer take it as a block and the plot stays a plot; `victimClass` on the file → `arrestChance` + 0.05/0.15, CASE_MONTHS_RICH 12, one step up the sentence table for a theft from the ultrawealthy and the hall for their murder. Art `js/art/estate.js` (the mansion and the plot, both zooms, lit and marked). 913 checks (60 in `tools/check-wealth.mjs`), 16/16 mutants, six mayor rigs byte-identical, browser-verified. `tools/wealthprobe.mjs`: on the estate layout the mansion rose in month 15 (the shops rung bound), taken at once; on the balanced layout the quarter does not fit.
- [ ] **Open after the build:** the CENTRE BOTTLENECK — a first theft from the ultrawealthy goes to the centre, and in the four-seed justice run 9 of the 11 cold ultrawealthy files had a roll succeed and wait for one of the centre's six beds (clearance 8% against 22% plain); a fallback to the cells, more beds, or a longer wait is the owner's call. TAX_CLASS [1, 2, 5] as proposed, now visible in the Budget and Census. Growth-made mansions (displacement), the affluent "kept" look, shops as victims, a class weight in the C valve and a species-skinned mansion set are not built, by the proposal's own sequencing.
- [ ] **Found in passing and FIXED in the same commit:** the campsite guard in `ops.costOf` listed the kinds through the centre and stopped — a Library, University, Gallery or Amphitheater could be dropped on an occupied tent; the list now carries every footprint op.
