...
- [ ] **Open after the rebuild:** MANSION_P 0.25 against the months a planned quarter waits; TAX_CLASS [1, 2, 5] (a progressive-tax Options switch is one line if wanted); a mansion can never rise INSIDE a dense block, by the crime law — the owner may want that or not; the species-skinned mansion set the owner welcomes; the centre bottleneck from the first build stands whenever an affluent file exists; the affluent "kept" look, shops as victims and a class weight in the C valve are not built.
