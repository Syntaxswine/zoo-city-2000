...
- **City builder:** use the existing construction palette, WASD/arrows to pan, right-drag to orbit and tilt, and scroll to zoom.
- **Walk the city:** choose a rabbit, fox, raccoon or bear. WASD/arrows walk, Shift runs, right-drag turns the camera, and E opens a nearby resident's real life/needs card. Buildings, trees and water block walking; bridges are walkable.
- **F2** switches between the two modes. The city continues running; Space pauses its simulation while you explore.
- Saves retain the original city format. The explorer is a separate visitor; its position and appearance are session-only and do not change the resident roster or simulation RNG.

The demo is a ten-year town generated by the existing scripted mayor. Missions, driving, interiors, and player theft mechanics are not implemented. Original sprite art and CPU-renderer checks remain as legacy assets and regression coverage; the browser now draws real depth-tested 3D geometry through `js/three/renderer.js`. Detailed sprite-specific visual features described below are records of the original game, not full 3D visual parity.

`npm run check` includes the simulation suites and `tools/check-three.mjs`, which verifies perspective picking, collision, bridges, boundaries, and simulation isolation.

## Original simulation and design record

A city builder where anthropomorphic animals live and grow
...
**Original Zoo City 2000:** https://syntaxswine.github.io/zoo-city-2000/
